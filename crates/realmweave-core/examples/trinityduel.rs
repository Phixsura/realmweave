...
fn main() {
    let games: u32 = std::env::args().nth(1).and_then(|s| s.parse().ok()).unwrap_or(4);
    let side: usize = std::env::args().nth(2).and_then(|s| s.parse().ok()).unwrap_or(11);
    for g in 0..games {
        ...
        while game.result().is_none() {
            ...
            let Some(mv) = bot::choose_move(&game, seed) else { break };
            if game.play(mv).is_err() { break; }
