...
impl Tri {
    ...
    fn neighbors(&self, id: usize) -> Vec<usize> {
        ...
        let get = |rr: i64, cc: i64| -> Option<usize> {
            if rr < 0 || rr >= self.n as i64 || cc < 0 || cc > rr {
                None
            } else {
                Some(self.index[rr as usize][cc as usize])
            }
        };
...
fn main() {
    ...
    let tri = Tri::new(n);
    println!(
        "triangle side {n}: {} cells per realm, {} total (3 realms)",
        tri.cells(),
        tri.cells() * 3
    );
    ...
    let t = trials as f64;
    println!(
        "single realm Y: P1 {:.1}% | P2 {:.1}% | NOBODY {:.2}% (theorem says 0)",
        100.0 * p1 as f64 / t,
        100.0 * p2 as f64 / t,
        100.0 * dead as f64 / t
    );
    ...
    println!(
        "match (majority of 3 realms): P1 {:.1}% | P2 {:.1}% | draws 0.0% BY CONSTRUCTION",
        100.0 * m1 as f64 / t,
        100.0 * m2 as f64 / t
    );
}
