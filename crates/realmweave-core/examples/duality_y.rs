...
fn y_connected(board: &BoardGraph, occ: &[Option<Player>], player: Player, sides: &[Vec<NodeId>; 3]) -> bool {
    ...
    for start in 0..n as NodeId {
        ...
        for m in &members {
            for k in 0..3 {
                if sides[k].contains(m) { touch[k] = true; }
            }
        }
...
fn run(board: &BoardGraph, sides_l: &[Vec<NodeId>; 3], sides_d: &[Vec<NodeId>; 3], trials: u32, label: &str) {
    let n = board.node_count();
    ...
    let t = trials as f64;
    println!("{label}: one {:.1}% | both {:.1}% | neither {:.1}%",
        100.0 * one as f64 / t, 100.0 * both as f64 / t, 100.0 * neither as f64 / t);
}
...
fn welded(size: usize) -> BoardGraph {
    ...
    for i in 0..realm_size {
        ...
        for (a, b) in [(h, m), (m, u)] {
            let exists = def.edges.iter().any(|e| (e.a == a && e.b == b) || (e.a == b && e.b == a));
            if !exists { def.edges.push(Edge { a, b, kind: EdgeKind::Portal }); }
        }
    }
...
fn main() {
    ...
    run(&std_board, &g1, &g1, trials, "G1 single-realm Y (shared sides 0/2/4)");
    ...
    run(&wb, &g2, &g2, trials, "G2 welded trinity-Y (H0/M2/U4 shared)");

    // G3: welded board, Y where each side-set = that side in ALL realms.
    let g3 = [
        [side_nodes(&wb, Realm::Heaven, 0, radius), side_nodes(&wb, Realm::Mortal, 0, radius), side_nodes(&wb, Realm::Underworld, 0, radius)].concat(),
        [side_nodes(&wb, Realm::Heaven, 2, radius), side_nodes(&wb, Realm::Mortal, 2, radius), side_nodes(&wb, Realm::Underworld, 2, radius)].concat(),
        [side_nodes(&wb, Realm::Heaven, 4, radius), side_nodes(&wb, Realm::Mortal, 4, radius), side_nodes(&wb, Realm::Underworld, 4, radius)].concat(),
    ];
