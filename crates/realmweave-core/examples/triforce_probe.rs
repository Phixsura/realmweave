...
fn main() {
    ...
    println!(
        "triforce side {n}: {cells} cells; regions sized {:?}",
        (0..4)
            .map(|k| (0..cells).filter(|&i| tri.region(i) == k).count())
            .collect::<Vec<_>>()
    );
    let trials = 3000;
        ...
        'outer: for pl in [1u8, 2] {
            ...
            for start in 0..cells {
                ...
                if touch == 7 {
                    ...
                    if regions.iter().filter(|&&x| x < 3).count() >= 2 {
                        crossers += 1;
                    }
                    if regions.contains(&3) {
                        heart_users += 1;
                    }
                    break 'outer;
                }
    ...
    let t = trials as f64;
    println!(
        "decisive: {:.2}% (P1 {:.1}% / P2 {:.1}%), dead {:.2}%",
        100.0 * (p1 + p2) as f64 / t,
        100.0 * p1 as f64 / t,
        100.0 * p2 as f64 / t,
        100.0 * dead as f64 / t
    );
    println!(
        "winning group crosses >=2 realms: {:.1}%",
        100.0 * crossers as f64 / (p1 + p2) as f64
    );
    println!(
        "winning group uses the heart:     {:.1}%",
        100.0 * heart_users as f64 / (p1 + p2) as f64
    );
}
