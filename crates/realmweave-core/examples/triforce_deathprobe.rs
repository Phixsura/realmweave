...
impl Tri {
    fn new(n: usize) -> Self {
        ...
        for r in 0..n {
            let mut row = Vec::new();
            for c in 0..=r { row.push(coords.len()); coords.push((r, c)); }
            index.push(row);
        }
        Tri { n, coords, index }
    }
...
fn group_alive(tri: &Tri, occ: &[u8], start: usize) -> (Vec<usize>, bool) {
    ...
    while let Some(cur) = q.pop_front() {
        for nb in tri.neighbors(cur) {
            if occ[nb] == 0 { alive = true; }
            else if occ[nb] == pl && !seen[nb] { seen[nb] = true; q.push_back(nb); members.push(nb); }
        }
    }
    (members, alive)
}
    ...
    for _ in 0..games {
        ...
        let winner = loop {
            ...
            for _try in 0..cells * 2 {
                ...
                if occ[id] != 0 || ko == Some(id) { continue; }
                // own eye skip
                ...
                if !alive { occ[id] = 0; for &m in &captured { occ[m] = 3 - to_move; } caps -= captured.len() as u32; continue; }
                ko = if captured.len() == 1 { Some(captured[0]) } else { None };
                placed = true;
                break;
            }
            if !placed { break 0; } // no legal move found (rare)
            to_move = 3 - to_move;
            moves += 1;
        };
        if winner == 0 { undecided += 1; }
        lens.push(moves);
        caps_total += caps;
    }
    lens.sort_unstable();
